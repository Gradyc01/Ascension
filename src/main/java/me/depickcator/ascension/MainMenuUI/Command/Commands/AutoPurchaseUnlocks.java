...
public class AutoPurchaseUnlocks implements Commands {
    ...
    public void uponEvent(InventoryClickEvent event, PlayerData playerData) {
        ...
        Component bool;
        if (!playerData.getPlayerStats().getSetting(PlayerStats.autoPurchaseUnlocks)) {
            bool = TextUtil.makeText("Enabled", TextUtil.GREEN);
        } else {
            bool = TextUtil.makeText("Disabled", TextUtil.RED);
        }
        playerData.getPlayerStats().booleanSwitch(PlayerStats.autoPurchaseUnlocks);
        p.sendMessage(TextUtil.makeText("Auto Purchase Unlocks is now ", TextUtil.AQUA).append(bool));
    ...
    private ItemStack makeButton() {
        ...
        itemMeta.displayName(TextUtil.makeText("Auto Purchase Unlocks", TextUtil.AQUA));
